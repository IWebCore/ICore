...
namespace IStringUtil
{
    ...
    inline std::string mid(const std::string& str, size_t start, long long length = -1) {
        if (start >= str.size()) {
    ...
    inline std::string left(const std::string& str, long long length = -1) {
    ...
    inline std::string right(const std::string& str, long long length = -1) {
        if (length == -1 || length >= static_cast<long long>(str.size())) {
            return str;  // 如果 length 是 -1 或者大于等于字符串长度，返回整个字符串
        }
